...
public:
	...
	std::vector<ChatCommand> GetCommands() const override
	{
		static std::vector<ChatCommand> commandTable =
		{
			...
			//{ "enchant", rbac::RBAC_PERM_COMMAND_AURA, false, &HandleEnchantCommand, "" },
	...
	{
		// token to 0, if private item = 1, and if item = 1, we send message to the staff.
		uint32 privateItem = 0;
	...
	*/
	...
	/*static bool HandleEnchantCommand(ChatHandler* handler, char const* args)
	{
	if (!*args)
	return false;
	...
	ObjectGuid guid = handler->extractGuidFromLink(strtok((char*)args, " "));
	uint32 enchantId = uint32(atoi(py));


	Item* item1 = handler->GetSession()->GetPlayer()->GetItemByGuid(guid);
	...
	// Get Player
	Player* item_owner = handler->GetSession()->GetPlayer();
	if (!item_owner)
	return false;


	// remove old enchanting before applying new if equipped
	item_owner->ApplyEnchantment(item1, PERM_ENCHANTMENT_SLOT, false);

	item1->SetEnchantment(PERM_ENCHANTMENT_SLOT, enchantId, 0, 0, item_owner->GetGUID());

	return true;

	// add new enchanting if equipped
	//item_owner->ApplyEnchantment(item1, PERM_ENCHANTMENT_SLOT, true);

	//item_owner->RemoveTradeableItem(item1);
	//item1->ClearSoulboundTradeable(item_owner);
